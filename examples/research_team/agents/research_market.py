from dataclasses import asdict

from sulcus import AgentProcess

from ..contracts import ResearchAssignment, ResearchResult
from ..data import MARKET_TRENDS
from ..runtime_events import record_agent_work


class ResearchMarketAgent(AgentProcess):
    name = "Research-Market"

    async def on_message(self, message) -> None:
        with record_agent_work(self):
            assignment = ResearchAssignment(**message.payload["assignment"])
            if assignment.focus_area != "Market Trends":
                raise ValueError(f"unexpected focus area: {assignment.focus_area}")
            self.assignment_received = assignment
            print(f"[Research-Market] Received assignment: {assignment.focus_area}")
            result = ResearchResult(focus_area=assignment.focus_area, findings=MARKET_TRENDS)
            self.result_sent = result
            print(f"[Research-Market] Produced {len(result.findings)} findings")
            print("[Research-Market] Sending result -> SynthesizerAgent")
            self.send(self.synthesizer_pid, {"result": asdict(result)})
